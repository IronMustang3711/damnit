...
OI::OI() {
	...
	clamp2Switch->WhenPressed(new ClampTiltPosition(206)); // 215));   // clamp to switch %rod15
	clamp2Down.reset(new frc::JoystickButton(controlJoystick.get(), 4));
	clamp2Down->WhenPressed(new ClampTiltPosition(336)); //357)); // clamp down to pickup cube %rod15
	clamp2Bucket.reset(new frc::JoystickButton(controlJoystick.get(), 5));
	clamp2Bucket->WhenPressed(new ClampTiltPosition(82)); // 84)); //	clamp to bucket %rod
